...
const init = reactive({
  ...
  setup: function (editor) {
    ...
    editor.on('NodeChange', function (e) {
      ...
      // 考虑到样式值可能以不同的格式出现，进行一些转换和判断
      if (bgColor === 'rgb(255, 255, 255)' || bgColor === '#ffffff' || bgColor === 'white') {
        bgColor = '#ffffff' // 统一为 #ffffff 方便判断
      }
      ...
      if (
        ...
        color = 'rgba(0, 0, 0, 0.9)' // 统一为 rgba(0, 0, 0, 0.9) 方便判断
      }

      // 检查是否符合要移除的条件：背景为白色，字体为黑色或接近黑色的 rgba 值
      if (bgColor === '#ffffff' && color === 'rgba(0, 0, 0, 0.9)') {
        // 移除这两个行内样式
